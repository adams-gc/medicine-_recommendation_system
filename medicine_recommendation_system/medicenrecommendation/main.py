...
from database import get_db_connection, get_all_symptoms, add_feedback

import sqlite3
import os

# flask app
app = Flask(__name__)  # this is obj. of class
...
@app.route("/")
def index():
    return render_template("index.html") # rander template help to redirect to index
...
def dashboard():
    ...
    performance_data = [0.95, 0.90, 0.85, 0.87, 0.92]
    importance_data = [0.4, 0.3, 0.2, 0.1, 0.05]
    sensitivity_data = [0.8, 0.6, 0.9, 0.7]
...
def api_data():
    ...
    data = {
        'performance': [0.95, 0.90, 0.85, 0.87, 0.92],
        'importance': [0.4, 0.3, 0.2, 0.1, 0.05],
        'sensitivity': [0.8, 0.6, 0.9, 0.7]
    }
    return jsonify(data)
...
def symptoms():
    ...
    return jsonify([dict(symptom) for symptom in symptoms])
